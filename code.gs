function testjson(e){
  ...
  Logger.log( Utilities.jsonStringify(e) );
    ...
    var html = HtmlService.createTemplateFromFile('seder')
    html.sbook = js;
    //return HtmlService.createTemplateFromFile('seder').evaluate();
    return HtmlService;
...
function doGet(e) {
  ...
  if (!e.parameter.page) {
    // When no specific page requested, return "home page"
    var html = HtmlService.createTemplateFromFile('seder')
    html.sbook = e.parameter.book;
    html.seder = e.parameter.seder;
    html.sverse = e.parameter.verse;
    html.parasha = e.parameter.parasha;
...
function fillverse()
{
  var url = "https://docs.google.com/spreadsheets/d/1aI5zgF-ZqoRz7Pb2gU_G_TUOyqNVcesnBFuYN71NhwE/edit#gid=1699385220";
  var ss = SpreadsheetApp.openByUrl(url);
  var wsb = ss.getSheetByName("bible");
  var wss = ss.getSheetByName("seder");
    
  
  var dataword = wsb.getDataRange().getValues();
  var dataverse = wss.getDataRange().getValues();
  
  for(var j = 301000; j<dataword.length;j++){ //dataword.length
    var bookname = dataword[j][0];
    var chapter = dataword[j][1];
    var verse = dataword[j][2];
    //var line = ''
    for(var i = 1; i<dataverse.length;i++) //dataword.length
      {
        if(dataverse[i][0] == bookname && dataverse[i][1] ==chapter && dataverse[i][2] == verse){ //[1] because column B
          sbook=dataverse[i][3]
          seder=dataverse[i][4]
          sverse=dataverse[i][5]
      }
    }
    k=j+1
    wsb.getRange('F'+k).setValue(sbook);
    wsb.getRange('G'+k).setValue(seder);
    wsb.getRange('H'+k).setValue(sverse);
  }
...
function nonikud(){
 var url = "https://docs.google.com/spreadsheets/d/1aI5zgF-ZqoRz7Pb2gU_G_TUOyqNVcesnBFuYN71NhwE/edit#gid=1699385220";
  var ss = SpreadsheetApp.openByUrl(url);
  var wsb = ss.getSheetByName("bible");
  var data = wsb.getDataRange().getValues();
  for(var j = 0; j<data.length;j++){ 
    var word = data[j][3];    
    newword = ""
    for (var i = 0;i<word.length;i++)
    {
      if (word[i]=='א') newword = newword + word[i]
      if (word[i]=='ב') newword = newword + word[i]
      if (word[i]=='ג') newword = newword + word[i]
      if (word[i]=='ד') newword = newword + word[i]
      if (word[i]=='ה') newword = newword + word[i]
      if (word[i]=='ו') newword = newword + word[i]
      if (word[i]=='ז') newword = newword + word[i]
      if (word[i]=='ח') newword = newword + word[i]
      if (word[i]=='ט') newword = newword + word[i]
      if (word[i]=='י') newword = newword + word[i]
      if (word[i]=='כ') newword = newword + word[i]
      if (word[i]=='ך') newword = newword + word[i]
      if (word[i]=='ל') newword = newword + word[i]
      if (word[i]=='מ') newword = newword + word[i]
      if (word[i]=='ם') newword = newword + word[i]
      if (word[i]=='נ') newword = newword + word[i]
      if (word[i]=='ן') newword = newword + word[i]
      if (word[i]=='ס') newword = newword + word[i]
      if (word[i]=='ע') newword = newword + word[i]
      if (word[i]=='פ') newword = newword + word[i]
      if (word[i]=='ף') newword = newword + word[i]
      if (word[i]=='צ') newword = newword + word[i]
      if (word[i]=='ץ') newword = newword + word[i]
      if (word[i]=='ק') newword = newword + word[i]
      if (word[i]=='ר') newword = newword + word[i]
      if (word[i]=='ש') newword = newword + word[i]
      if (word[i]=='ת') newword = newword + word[i]
      
    }
    k = j+1;
    wsb.getRange('E'+k).setValue(newword);
  }
